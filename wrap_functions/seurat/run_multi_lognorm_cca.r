...
option_list <- list(
  ...
  make_option(c("--min_pct"), type = "double", default = 0.1,
              action = "store", help = "This is min.pct used in FindAllMarkers function , default is 0.1"
  ),
...
for (i in 1:length(object.list)) {
  object.list[[i]] <- NormalizeData(object.list[[i]], verbose = FALSE, scale.factor = opt$scale_factor)
...
object.anchors <- FindIntegrationAnchors(object.list = object.list, dims = 1:opt$npc_used,
                                         normalization.method = "LogNormalize") # here
object.integrated <- IntegrateData(anchorset = object.anchors, dims = 1:opt$npc_used,
